...
![](elements/nature_2021.png){.cap-center fig-align="center" height=500px}
...
- AI can (magically) accurately predict the `rank` of the elliptic curve, but why?
  ...
  - Leading to progress toward “explaining why”.
...
## Last iterate convergence (theorem) 
...
:::
::: {.column width="50%" style="font-size: 0.7em;"}
- Question: $\lim_{k\to \infty} x_k = x_\infty$?
...
- GD (slow convergence, convergent)
  ...
  f(z_{k}) = \mathcal{O}(\frac{1}{k}), \\
    \lim_{k\to \infty} z_k = z_\infty \in \mathrm{argmin} f
  \end{aligned}
  $$
- NAG (fast convergence, oscillation)
  ...
  f(x_{k}) = \mathcal{O}(\frac{1}{k^2}), \\
    \lim_{k\to \infty} x_k = x_\infty \in \mathrm{argmin}\ f\ ?
  \end{aligned}
  $$
:::
...
## Strategy {style="font-size: 0.6em;"}
...
- Known results
  - $\gamma > 3$, we have $\lim_{t \to \infty} X(t) = X(\infty) \in \mathrm{argmin} f.$ 
    ...
    $$
      \int_0^\infty t(f(X(t)) - f^\star) dt < \infty\quad  \text{ or } \quad  \int_0^\infty t\|\dot X(t)\|^2 dt < \infty.
    $$
  - $\gamma = 3$ critical case, corresponds to NAG. 
    - The integral of the above energy function diverges.
- Strategy: Ask AI to propose a new energy function that is integrable.
  - Once the continous time model converges, same idea applies to the discrete time dynamics.
  - This is not always the case. For example, see one of our open problems.
...
## Suggestions, Prompting Techniques, and Pitfalls {style="font-size: 0.7em;"}
<div style="height: 0.8em;"></div>

::: {.columns}
::: {.column width="50%" style="font-size: 0.7em;"}
- Suggestions
  - AI is a collaborator, but you should lead the project
    - Ask AI to help you to understand the problem.
    - Literature review using AI. Understand the existing attempts.
    - Understand the key challenge. Ideally summarize it to a very detailed mathematical problem.
    - AI only fills small gaps. You need a big picture in mind.
  
- Techniques
  - As much details as possible.
  - Provide context (upload files) if it helps
  - Role play: "You are a top expert in optimization. I am working on this open problem. This is a field that I am not familiar with. Please walk me through the problem description in a detailed and highly accessible way. Break it down to the basics in calculas and linear algebra. ..."
...
- Pitfalls (personal experience)
  - **DO NOT** trust AI. It will hallucinate! For example
    - sign error (mixing $+$ with $-$);
    - mixing functions (confusing Bessel functions of the first kind with the second kind);
    - overly optimistic and ignore non-trivial steps.
  - Verify every step (with the help of AI).
  - Avoid long context
    - Do not ask all questions in a single conversation. 
    - Restarting conversation leads to new ideas.
...
<!-- ## Open Problem Collections {style="font-size: 0.7em;"}
...
::: {.column width="60%"}
![](elements/list%20of%20problems.png){fig-align="center" height=600px}
:::
...
- Useful links
  - Course Description
    ...
    ![](elements/description.png){fig-align="center" height=90px}    

  - Logistics
  
    ![](elements/logistics.png){fig-align="center" height=90px}

  - Signup Sheet
  
    ![](elements/signup.png){fig-align="center" height=90px}
